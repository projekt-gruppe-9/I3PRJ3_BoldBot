/*******************************************************************************
* File Name: Right_calibrator_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_Start
********************************************************************************
*
* Summary:
...
void Right_calibrator_theACLK_Start(void) 
{
    ...
    Right_calibrator_theACLK_CLKEN |= Right_calibrator_theACLK_CLKEN_MASK;
	Right_calibrator_theACLK_CLKSTBY |= Right_calibrator_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_Stop
********************************************************************************
*
* Summary:
...
void Right_calibrator_theACLK_Stop(void) 
{
    ...
    Right_calibrator_theACLK_CLKEN &= (uint8)(~Right_calibrator_theACLK_CLKEN_MASK);
	Right_calibrator_theACLK_CLKSTBY &= (uint8)(~Right_calibrator_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Right_calibrator_theACLK_CLKEN & Right_calibrator_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Right_calibrator_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Right_calibrator_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Right_calibrator_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Right_calibrator_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Right_calibrator_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Right_calibrator_theACLK_GetSourceRegister();
    uint16 oldDivider = Right_calibrator_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Right_calibrator_theACLK_CLKEN & Right_calibrator_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Right_calibrator_theACLK_DIV_PTR, clkDivider);
                Right_calibrator_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Right_calibrator_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Right_calibrator_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Right_calibrator_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Right_calibrator_theACLK_DIV_PTR, clkDivider);
				Right_calibrator_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Right_calibrator_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Right_calibrator_theACLK_MOD_SRC |= modeBitMask & (uint8)Right_calibrator_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Right_calibrator_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Right_calibrator_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Right_calibrator_theACLK_MOD_SRC & (uint8)(Right_calibrator_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Right_calibrator_theACLK_GetDividerRegister();
    uint8 oldSrc = Right_calibrator_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Right_calibrator_theACLK_MOD_SRC |= CYCLK_SSS;
        Right_calibrator_theACLK_MOD_SRC =
            (Right_calibrator_theACLK_MOD_SRC & (uint8)(~Right_calibrator_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Right_calibrator_theACLK_MOD_SRC & Right_calibrator_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Right_calibrator_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Right_calibrator_theACLK_PHASE = clkPhase & Right_calibrator_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Right_calibrator_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Right_calibrator_theACLK_PHASE & Right_calibrator_theACLK_PHASE_MASK;
}
