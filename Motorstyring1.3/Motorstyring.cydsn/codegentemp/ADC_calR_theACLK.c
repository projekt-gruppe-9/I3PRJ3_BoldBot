...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    ADC_calR_theACLK_CLKEN |= ADC_calR_theACLK_CLKEN_MASK;
	ADC_calR_theACLK_CLKSTBY |= ADC_calR_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ADC_calR_theACLK_CLKEN &= (uint8)(~ADC_calR_theACLK_CLKEN_MASK);
	ADC_calR_theACLK_CLKSTBY &= (uint8)(~ADC_calR_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_StopBlock(void) 
{
    if ((ADC_calR_theACLK_CLKEN & ADC_calR_theACLK_CLKEN_MASK) != 0u)
    {
...
#if defined(ADC_calR_theACLK__CFG3)
        CLK_DIST_AMASK = ADC_calR_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_calR_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_calR_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_calR_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_calR_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ADC_calR_theACLK_CLKSTBY &= (uint8)(~ADC_calR_theACLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ADC_calR_theACLK_GetSourceRegister();
    uint16 oldDivider = ADC_calR_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_calR_theACLK_CLKEN & ADC_calR_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_calR_theACLK_DIV_PTR, clkDivider);
                ADC_calR_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_calR_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_calR_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_calR_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_calR_theACLK_DIV_PTR, clkDivider);
				ADC_calR_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ADC_calR_theACLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(ADC_calR_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_SetModeRegister(uint8 modeBitMask) 
{
    ADC_calR_theACLK_MOD_SRC |= modeBitMask & (uint8)ADC_calR_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_ClearModeRegister(uint8 modeBitMask) 
{
    ADC_calR_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_calR_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_calR_theACLK_GetModeRegister(void) 
{
    return ADC_calR_theACLK_MOD_SRC & (uint8)(ADC_calR_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ADC_calR_theACLK_GetDividerRegister();
    uint8 oldSrc = ADC_calR_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_calR_theACLK_MOD_SRC |= CYCLK_SSS;
        ADC_calR_theACLK_MOD_SRC =
            (ADC_calR_theACLK_MOD_SRC & (uint8)(~ADC_calR_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_calR_theACLK_GetSourceRegister(void) 
{
    return ADC_calR_theACLK_MOD_SRC & ADC_calR_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_calR_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_calR_theACLK_SetPhaseRegister(uint8 clkPhase) 
{
    ADC_calR_theACLK_PHASE = clkPhase & ADC_calR_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_calR_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_calR_theACLK_GetPhaseRegister(void) 
{
    return ADC_calR_theACLK_PHASE & ADC_calR_theACLK_PHASE_MASK;
}
